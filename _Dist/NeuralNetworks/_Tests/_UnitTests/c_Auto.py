...
class TestAutoNN(unittest.TestCase):
    ...
    def test_09_predict_from_txt(self):
        self.assertIs(
            auto_mushroom_nn.predict_from_file(
                mushroom_file, "txt", include_label=True
            ).dtype, np.dtype("float32"), msg="Predict failed"
        )
        self.assertIs(
            auto_mushroom_nn.predict_classes_from_file(
                mushroom_file, "txt", include_label=True
            ).dtype, np.dtype("int32"), msg="Predict classes failed"
        )
        predict_labels = np.unique(auto_mushroom_nn.predict_labels_from_file(
            mushroom_file, "txt", include_label=True
        ))
    ...
    def test_15_predict_from_csv(self):
        self.assertIs(
            auto_adult_nn.predict_from_file(
                adult_file, "csv", include_label=True
            ).dtype, np.dtype("float32"), msg="Predict failed"
        )
        self.assertIs(
            auto_adult_nn.predict_classes_from_file(
                adult_file, "csv", include_label=True
            ).dtype, np.dtype("int32"), msg="Predict classes failed"
        )
    ...
    def test_21_predict_from_mixed_features(self):
        self.assertIs(
            auto_lmgpip_nn.predict_from_file(
                lmgpip_test_file, "txt", include_label=False
            ).dtype, np.dtype("float32"), msg="Predict failed"
        )
        self.assertIs(
            auto_lmgpip_nn.predict_classes_from_file(
                lmgpip_test_file, "txt", include_label=False
            ).dtype, np.dtype("int32"), msg="Predict classes failed"
        )
    ...
    def test_27_predict_regressor_from_mixed_features(self):
        self.assertIs(
            auto_lmgpip_regressor.predict_from_file(
                lmgpip_test_file, "txt", include_label=False
            ).dtype, np.dtype("float32"), msg="Predict failed"
        )
        with self.assertRaises(ValueError):
            auto_lmgpip_regressor.predict_classes_from_file(
                lmgpip_test_file, "txt", include_label=False
            )
