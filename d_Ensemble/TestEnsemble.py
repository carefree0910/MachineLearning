...
_clf_dic = {"AdaBoost": AdaBoost, "RF": RandomForest}
...
if __name__ == '__main__':
    _x, _y = gen_random()
    test(_x, _y)
    test(_x, _y, algorithm="RF")
    # test(_x, _y, clf="SKTree")
    # test(_x, _y, clf="SKTree", max_depth=1)
    # _x, _y = gen_xor()
    # test(_x, _y, clf="Cart")
    # test(_x, _y, clf="SKTree")
    # test(_x, _y, clf="Cart", max_depth=2, epoch=1)
    # test(_x, _y, clf="SKTree", max_depth=2, epoch=1)
    # test(_x, _y)
    # test(_x, _y, clf="SKTree")
    # _x, _y = gen_spin()
    # test(_x, _y, clf="SKTree")
